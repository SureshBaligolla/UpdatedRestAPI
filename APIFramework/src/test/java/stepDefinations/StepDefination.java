...
public class StepDefination extends Utils {
	 ...
	 * language, address)); }
	 * 
	 * @When("user calls {string} with {string} http request") public void
	 * user_calls_with_http_request(String resource, String method) { // Write code
	 * here that turns the phrase above into concrete actions //constructor will be
	 * called with value of resource which you pass APIResources
	 ...
	 * =res.when().post(resourceAPI.getResource()); else
	 ...
	 * in_response_body_is(String keyValue, String Expectedvalue) { // Write code
	 ...
	 * verify_place_Id_created_maps_to_using(String expectedName, String resource)
	 * throws IOException {
	 * 
	 * // requestSpec place_id=getJsonPath(response,"place_id"); res =
	 * given().spec(requestSpecification()).queryParam("place_id", place_id);
	 * user_calls_with_http_request(resource, "GET"); String actualName =
	 * getJsonPath(response, "name"); assertEquals(actualName, expectedName);
	 * 
	 * }
	 * 
	 * @Given("DeletePlace Payload") public void deleteplace_Payload() throws
